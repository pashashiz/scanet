...
trait Optimizer[A] {
  ...
  def minimize[B: DiffFunction](op: A, f: DFBuilder[B], coef: DenseMatrix[Double], initVars: DenseVector[Double]): Stream[IO, Event[B]] =
    optimize(op, f, coef, initVars, min = true)

  def maximize[B: DiffFunction](op: A, f: DFBuilder[B], coef: DenseMatrix[Double], initVars: DenseVector[Double]): Stream[IO, Event[B]] =
